...
void applyrules(Client* c) {
	...
	c->ignorehints = false;
	for (unsigned int i = 0; i < LENGTH(ignorehintsmatch); ++i) {
	...
	if (!c->ignorehints) {
		for (unsigned int i = 0; i < LENGTH(ignorehintscontains); ++i) {
...
void grabkeys(void) {
	...
	KeySym* syms = XGetKeyboardMapping(dpy, start, end - start + 1, &skip);
	if (!syms)
		return;
