...
static int drawstatusbar(Monitor *m, char* text);
		...
		if (text[i] == '^' && !isCode) {
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					buf[7] = '\0';
					drw_clr_create(drw, &drw->scheme[ColFg], buf, alpha);
					i += 7;
				} else if (text[i] == 'b') {
					char buf[8];
					memcpy(buf, (char*)text+i+1, 7);
...
drawbar(Monitor *m)
{
	...
	unsigned int i, occ = 0, n = 0;
	Client *c;

	tw = m->ww - drawstatusbar(m, stext);
	for (c = m->clients; c; c = c->next) {
		occ |= c->tags;
		if (ISVISIBLE(c))
			++n;
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, w, BH, lrpad / 2, tags[i], 0 << i);
		if (occ & 1 << i)
	...
	if ((w = m->ww - tw - x) > BH) {
		if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
				...
				if (c->isalwaysontop)
					drw_rect(drw, x + boxs, boxs + boxw + 1, boxw, boxw, 0, 0);
				x += tabw;
			}
		} else {
			drw_setscheme(drw, scheme[SchemeNorm]);
...
resize(Client *c, int x, int y, int w, int h)
{
	XWindowChanges wc;
	if      (w < BH      ) w = BH;
	else if (w < c->basew) w = c->basew;
	if      (h < BH      ) h = BH;
	else if (h < c->baseh) h = c->baseh;
...
resizemouse(const Arg *arg)
{
	...
	do {
		...
		case MotionNotify:
			if ((ev.xmotion.time - lasttime) <= (1000 / 60)) continue;
			lasttime = ev.xmotion.time;
			nw = ev.xmotion.x - ocx + 1;
			nh = ev.xmotion.y - ocy + 1;
...
setup(void)
{
	...
	scheme[LENGTH(colors)] = drw_scm_create(drw, colors[0], alphas[0], 2);
